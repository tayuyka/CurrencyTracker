...
import com.yuya.currencytracker.data.local.CurrencyDao
import com.yuya.currencytracker.data.local.toDomain
import com.yuya.currencytracker.data.local.toEntity
import com.yuya.currencytracker.data.local.toHistoryEntities
...
class RoomCurrencyRepository(
    ...
    override fun observeCurrencies(): Flow<List<Currency>> {
        return dao.observeCurrencies().map { currencies ->
            currencies.map { it.toDomain() }
        }
    }

    override suspend fun initialize() {
        if (dao.getCurrencies().isEmpty()) {
    ...
    override suspend fun getCurrencies(): List<Currency> {
        initialize()
        return dao.getCurrencies().map { it.toDomain() }
    }

    override suspend fun getCurrency(currencyId: String): Currency? {
        initialize()
        return dao.getCurrency(currencyId)?.toDomain()
    }

    override suspend fun saveCurrencies(currencies: List<Currency>) {
        dao.replaceAll(
            currencies = currencies.map { it.toEntity() },
            history = currencies.flatMap { it.toHistoryEntities() }
        )
    }
